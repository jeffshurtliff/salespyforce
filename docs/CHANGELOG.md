...
- Several new methods have been introduced within the core object to perform various tasks:
    - The {py:meth}`salespyforce.core.Salesforce.delete` method performs DELETE API calls.
    - The {py:meth}`salespyforce.core.Salesforce.get_latest_api_version` method retrieves the 
      latest Salesforce API version.
    - The {py:meth}`salespyforce.core.Salesforce.retrieve_current_user_info` method retrieves 
    ...
    - Several new methods were added to check the user access for a specific record:
        - {py:meth}`salespyforce.core.Salesforce.can_access_record`
        - {py:meth}`salespyforce.core.Salesforce.can_read_record`
        - {py:meth}`salespyforce.core.Salesforce.can_edit_record`
        - {py:meth}`salespyforce.core.Salesforce.can_delete_record`
    - The {py:meth}`salespyforce.core.Salesforce.get_18_char_id` method converts a 15-character 
