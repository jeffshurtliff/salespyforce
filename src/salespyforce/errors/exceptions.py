...
class InvalidURLError(SalesPyForceError):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not (args or kwargs):
            ...
            custom_msg = f"{default_msg.split('is')[0]}'{kwargs[_EXCEPTION_CLASSES._URL]}'{default_msg.split('URL')[1]}"
            args = (custom_msg,)
        super().__init__(*args)
...
class UnknownFileTypeError(SalesPyForceError):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not (args or kwargs):
            ...
            delimiter = 'path'
            custom_msg = f"{default_msg.split(delimiter)[0]}'{kwargs[_EXCEPTION_CLASSES._FILE]}'{default_msg.split(delimiter)[1]}"
            args = (custom_msg,)
        super().__init__(*args)
        ...
        if _EXCEPTION_CLASSES._IDENTIFIER in kwargs or _EXCEPTION_CLASSES._FEATURE in kwargs:
            ...
            if _EXCEPTION_CLASSES._FEATURE in kwargs:
                exc_msg = exc_msg.replace(
                    _EXCEPTION_CLASSES._FEATURE, f'{kwargs[_EXCEPTION_CLASSES._FEATURE]} {_EXCEPTION_CLASSES._FEATURE}'
                )
            args = (exc_msg,)
        elif not (args or kwargs):
            args = (exc_msg,)
